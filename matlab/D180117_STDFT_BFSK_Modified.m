...
DISPLAY_STUFF               = 1;
USE_SPECIAL_MAGNITUDE_SYNC  = 0;
USE_SPECIAL_MAGNITUDE_DETEC = 0;
USE_FFT_ZP_8_56             = 0;
...
ENABLE_1FP  = (1 | ALL_FP_ENABLED) & ~DISABLE_ALL_FP;
FP1_f       = 4;
FP1_i       = 4; % biggest input magnitude R8.9926 I9.0724
FP1_w       = FP1_i+FP1_f;

% FPfft FFT ZP
ENABLE_FPfft = (1 | ALL_FP_ENABLED) & ~DISABLE_ALL_FP;
FPfft_f      = 7;
FPfft_i      = 5; % between 3 and 5 (biggest input magnitude R6.1499 I7.2534)
FPfft_w      = FPfft_i+FPfft_f;

% 2FP magnitude, unsigned
ENABLE_2FP  = (1 | ALL_FP_ENABLED) & ~DISABLE_ALL_FP;
FP2_f       = 2;
FP2_i       = 7; % biggest input magnitude R25.656 I26.95
FP2_w       = FP2_i+FP2_f;
    
% 3FP accumulator, unsigned
ENABLE_3FP  = (1 | ALL_FP_ENABLED) & ~DISABLE_ALL_FP;
FP3_f       = 2;
FP3_i       = 6;  % biggest input 19.533
FP3_w       = FP3_i+FP3_f;

% 4FP R, unsigned
ENABLE_4FP  = (1 | ALL_FP_ENABLED) & ~DISABLE_ALL_FP;
FP4_f       = 0;
FP4_i       = 7;  % biggest input 96.238
...
ebn             = [2:14];         % range of Eb/N0
NoOfPckts       = 10;           % #of packets
            ...
            if LOAD_COMPLETE_SYS==0
                if LOAD_DATA_PREAMBLE_DETECTOR==0
                    ...
                    for i = 1:NumOfDelays
                        ...
                        tmpcoll(q,j,i,1) = maxodd(i);
                        tmpcoll(q,j,i,1) = maxeven(i);
    ...
    %**************************************************************************
        ...
        %********************** Comparison and Decision making*********************                
                        if LOAD_DATA_GOERTZEL==0
                            % FPgrz
                            grzL = goertzel_mod_kprim(WindowedData, iL(q,j), ENABLE_FPgrz, FPgrz_w, FPgrz_i);
                            grzH = goertzel_mod_kprim(WindowedData, iH(q,j), ENABLE_FPgrz, FPgrz_w, FPgrz_i);
                        else
...
%||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
    ...
    if isempty(who('reference_ber')) %plot the theoretical curve only once
        ...
        semilogy(1:14, reference_ber,'LineWidth',2.0);
        xlabel('E_b/N_0 [dB]');
        ylabel('BER');
        hold on;
        semilogy(ebn, berfinal,'DisplayName',string(FP5_w),'LineWidth',1.4);
        semilogy(ebn, berfinal,'LineWidth',1.4);
    else
        semilogy(ebn, berfinal,'DisplayName',string(ADC_bit_resolution),'LineWidth',1.4);
        semilogy(ebn, berfinal,'LineWidth',1.4);
    end

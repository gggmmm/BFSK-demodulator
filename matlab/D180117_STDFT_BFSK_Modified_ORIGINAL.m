...
FP2_w       = 7;
FP2_i       = 6; % max value in input is 25 for both real and imag
...
%************************Generating Transmitted Signal********************
        ...
        sig_delay=0;
        Txsignal=[zeros(sig_delay,1);signal(1:end-sig_delay)];% or  [signal(sig_delay+1:end-4);zeros(sig_delay,1)];   %%%MODIFIED%%%%%
%**************************************************************************
...
%||||||||||||||||||||||||||||%%%%%%%MODIFIED%%%%%%%%%
            ...
            ber(q,j) = min(error1,error2)/(LengthDataPckt-2-PreambleLength);
%||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
        end
    end
